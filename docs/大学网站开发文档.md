...
- 站内聊天（V8 迁移新增 `chat_message`）：学生中心 `/student/chat` 与教师中心 `/instructor/chat` 提供一对一在线聊天（原生 WebSocket `/ws/chat` + JSON 协议，前端零依赖）。消息落库可追溯，支持历史记录（打开会话自动标记已读）、未读计数、在线状态、**清空聊天记录**（双向删除并实时同步双方界面）；联系人三入口：搜索框（`/chat/users` 关键字/院系/角色综合查询，服务端限流）、**常用联系人智能分组**（学生→我的同学/我的老师，教师→我的学生/我的同事，基于选课/授课关系）、**按院系浏览**（`/chat/departments` 院系列表+人数，组内可再搜索）；WebSocket 握手复用登录会话鉴权并校验同源 Origin（防跨站 WebSocket 劫持），消息内容按文本渲染防 XSS。
- 在线用户统计（会话监听器）：`SessionListener`（HttpSessionListener）配合 `OnlineUserTracker` 追踪在线状态——登录成功后注册（sessionId→用户），退出/超时/容器回收自动注销；管理端 `/admin/online` 可查看在线人数与明细（账号/姓名/类型/登录时间），首页统计卡片实时展示，页面每 30 秒轮询 `/admin/online/count` 刷新。多实例模式存储走 Redis（见下），单实例为内存实现。
- 多实例支持（Redis）：`app.multi-instance.enabled=true`（默认）时三处共享状态走 Redis——**在线用户统计**（`online:session:*`，带 TTL，实例宕机自动清理）、**登录限流**（`login:fail:*` ZSET 时间窗 + `login:lock:*` TTL 锁定）、**聊天跨实例路由**（消息经 Redis Pub/Sub 频道 `chat:message`/`chat:presence` 广播，接收方所在实例投递；在线集合存 `chat:online` SET）；单实例部署可设 `MULTI_INSTANCE_ENABLED=false` 退回内存实现。Docker 部署自带 Redis 服务。
- 缓存（Spring Cache，`@Cacheable`）：多实例模式用 RedisCacheManager（JSON 序列化 + 各缓存区独立 TTL），单实例回退内存 CacheManager。缓存区：`home` 首页聚合（热门课程/最新开课班/院系/公告，60s）、`stats` 统计报表（系预算/工资/选课/成绩分布，5min）、`forumCategories` 论坛板块（5min，管理员新增/重命名/启停时主动失效）、`displayName` 显示名查询（10min，聊天发消息避免全表扫）。
- 全文检索（V12 迁移）：论坛帖子搜索（标题+正文）与聊天联系人姓名搜索改用 **MySQL FULLTEXT + ngram 解析器**（支持中文分词），查询 `MATCH...AGAINST`（布尔模式，关键字先清洗剔除运算符字符防语法注入）；登录账号走精确/前缀匹配（`=` / `LIKE 'kw%'`，走索引）；1 字符短关键字自动回退 LIKE。三处索引：`forum_post(title,content)`、`student(name)`、`instructor(name)`。
- HTTPS 部署：独立 `tls` profile（`application-tls.yml`）——`--spring.profiles.active=dev,tls` 后应用以 HTTPS 服务在 8443 端口（默认 HTTP 8080 不变）；自签名证书由 `scripts/gen-keystore.ps1` 生成（`keystore/university.p12`，生产环境替换为正式证书或经反向代理终结 TLS）；聊天 WebSocket 自动跟随协议切换 `wss://`。
- 日志脱敏：Logback 自定义转换器（`%mask` / `%maskEx`，`SensitiveDataUtil`）在日志输出环节统一掩码敏感信息，覆盖**消息与异常堆栈**——BCrypt 密码哈希、手机号、身份证号、`password=`/JSON 密码、JSESSIONID、CSRF token、Bearer token；业务代码无需改动，脱敏对全部日志（控制台 + application.log + error.log）生效。
- 学习论坛（V9 迁移新增 `forum_post`/`forum_reply`，V10/V11 增强）：所有学生/教师（及管理员）可发帖与回复（公开浏览无需登录，写操作需登录）。**动态板块**（V11 `forum_category`，管理员在 `/admin/forum-categories` 新增/重命名/启停，停用后新帖不可选存量帖保留）、**置顶/加精**（管理员操作，置顶优先）、**点赞**（`forum_like` 去重 + 冗余计数，详情页可展开点赞人列表）、**@提及**（正文/回复输入 `@账号` 渲染为徽标并生成**站内通知**（V11 `user_notification`，导航栏铃铛角标 + `/notifications` 通知页 + 已读管理））、**帖子编辑**（作者可编辑，V11 `forum_post_history` 保存每次编辑前快照，`/forum/{id}/history` 查看历史版本）；列表支持关键字搜索与板块筛选、分页、按活跃度排序；删除权限：作者本人或管理员；帖子删除/置顶/加精/板块维护记入审计日志（对象类型 FORUM）。
