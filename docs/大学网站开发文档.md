...
- 在线用户统计（会话监听器）：`SessionListener`（HttpSessionListener）配合 `OnlineUserTracker` 追踪在线状态——登录成功后注册（sessionId→用户），退出/超时/容器回收自动注销；管理端 `/admin/online` 可查看在线人数与明细（账号/姓名/类型/登录时间），首页统计卡片实时展示，页面每 30 秒轮询 `/admin/online/count` 刷新。单实例内存实现，多实例部署需改为 Redis 等共享存储。
- 学习论坛（V9 迁移新增 `forum_post`/`forum_reply`，V10 增强）：所有学生/教师（及管理员）可发帖与回复（公开浏览无需登录，写操作需登录）。支持**板块分类**（学习交流/课程答疑/校园生活/资源共享/意见建议，列表可筛选）、**置顶/加精**（管理员操作，置顶优先展示）、**点赞**（登录用户可切换，`forum_like` 去重 + 冗余计数）、**@提及**（正文/回复输入 `@账号` 自动渲染为提及徽标，服务端转义 + 解析显示名防 XSS）、**帖子编辑**（作者可改标题/正文/分类）；列表支持关键字搜索与分页、按活跃度排序；删除权限：作者本人或管理员（帖子删除连带回复）；帖子删除/置顶/加精记入审计日志（对象类型 FORUM）。
