...
public class Accumulator {
    ...
    public static void main(String[] args) {
        ...
        StdOut.println("请输入数字，输入非数字内容（如'quit'或'end'）结束输入：");

        while (!StdIn.isEmpty()) {  // 修复：使用 !StdIn.isEmpty() 替代 StdIn.hasNextDouble()
            try {
                double x = StdIn.readDouble();
                stats.addDataValue(x);
                StdOut.println("已添加: " + x + ", 当前统计: " + stats);
            }catch(Exception e){
                break;
            }
        }
